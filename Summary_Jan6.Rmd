...
DEEPSEAWATER: [DSW], 10% Deep Sea Water (DSW) taken from 700 m, filtered and evenly mixed with 90% in situ water from 25 m. This mixture was then incubated and filtered. 
...
```{r Which Compounds, include=TRUE}
...
which.compounds <- ggplot(Which.Cmpds, aes(x = reorder(Precursor.Ion.Name, -Percent.Present),
                        y = Percent.Present)) +
...
kable(Table.DF.Cyclonic)
kable(Table.DF.Anticyclonic)
...
glimpse(Vitamin.Complete.Set)
levels(Vitamin.Complete.Set$Binned.Group)

# Vitamin.Complete.Set.Anova <- lapply(split(Vitamin.Complete.Set, Vitamin.Complete.Set$Precursor.Ion.Name), function(i) {
#   aov(lm(Area.with.QC ~ Binned.Group, data = i))
# })
# Vitamin.Complete.Set.Summary <- lapply(Vitamin.Complete.Set.Anova, function(i) {
#   summary(i)
# })
# Vitamin.Complete.Set.AnovaDF <- as.data.frame(do.call(rbind, lapply(Vitamin.Complete.Set.Summary, function(x) {temp <- unlist(x)})))
