...
IS.Raw.Area.Plot <- ggplot(Int.Stds.data, aes(x = Replicate.Name, y = Area.with.QC)) +
  ...
  facet_wrap( ~Mass.Feature, scales = "fixed") +
...
test_IS_data <- as.data.frame(sort(unique(Int.Stds.data$Replicate.Name)), stringsAsFactors = FALSE)
test_long <- as.data.frame(sort(unique(Data.long$Replicate.Name)), stringsAsFactors = FALSE)
test <- identical(test_IS_data[[1]], test_long[[1]])
...
# Change the BMIS to "Inj_vol" if the BMIS is not an acceptable match --------------------------------------------------------------
...
IS_toISdat <- Mydata.new %>%
  ...
  filter(type == "Smp") %>%
  group_by(Mass.Feature, MIS) %>%
  mutate(RSD_of_Smp = sd(Adjusted.Area, na.rm = TRUE) / mean(Adjusted.Area, na.rm = TRUE)) %>%
